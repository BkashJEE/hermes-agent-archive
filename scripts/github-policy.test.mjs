...
test('one inclusive default accepts 1000 and rejects 999; overrides can lower or raise it',()=>{
  assert.equal(githubStarFloor(),1000);
  assert.equal(qualifiesStars(999),false);
  assert.equal(qualifiesStars(1000),true);
  assert.equal(qualifiesStars(500,githubStarFloor('500')),true);
...
test('rendering and Trending follow the fetched policy rather than a second floor',()=>{
  const now=Date.now();
  const live={githubMinStars:500,github:[{repo:'org/tool',stars:500,forks:1,url:'https://github.com/org/tool',observedAt:new Date(now-1000).toISOString(),previousStars:{value:400,at:new Date(now-86400000).toISOString()}}]};
